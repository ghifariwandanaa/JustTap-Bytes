...
<div class="card">
    <div class="card-body">
        ...
        <p class="card-text">Custom Link: 
            @if ($businessCard->custom_link)
                <a href="{{ $businessCard->custom_link }}" target="_blank">{{ $businessCard->custom_link }}</a>
            @else
                Tidak ada custom link
            @endif
        </p>
        <p class="card-text">QR Code: 
            <a href="{{ url('qrcode/' . $businessCard->id) }}" class="btn btn-info">Unduh QR</a>
        </p>
